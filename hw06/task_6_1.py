...
line_index = -1
max_line_sum = 0   #Не работает с отриц. числами
max_line_index = 0
...
'''Найти индекс колонки с максимальной суммой элементов.''' # можно поменять местами row и column
...
for index in matrix:             # Использовать enumerate, чтобы не было -1 в line_index_count
    line_sum = 0
    for elem in index:
        line_sum += elem
    line_index_count += 1
